...
experiments = [
    #{"consistency": "chain replication", "k": 1}
    #{"consistency": "chain replication", "k": 3}
  #  {"consistency": "chain replication", "k": 5}
  #  {"consistency": "eventual consistency", "k": 1}
  #{"consistency": "eventual consistency", "k": 3}
{"consistency": "eventual consistency", "k": 5}
]

def insert_keys_from_file(node_id, config):
    """
    Reads insert_<node_id>_part.txt and sends POST requests to the corresponding node's /insert endpoint.
    Each line in the file contains a song name. A fixed value is used for the insert.
    Returns a tuple: (number of successful inserts, time spent reading the file in seconds)
    """
    ...
    for line in lines:
        ...
        if song_name:
            ...
            url = f"http://{ip}:{port}/insert/{encoded_song}/{fixed_value}"
...
def run_experiment(config):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for node_id in node_mapping.keys():
            futures.append(executor.submit(insert_keys_from_file, node_id, config))
    ...
    print(f"Effective Insert Time (elapsed - reading): {effective_time:.2f} sec")
